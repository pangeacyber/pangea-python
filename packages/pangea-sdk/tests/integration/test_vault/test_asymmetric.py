...
def signature(vault, canonical_asymmetric_args, plain_text, test_name) -> str:
    return vault.sign(_asymmetric_key(vault,  "asymmetric", test_name, canonical_asymmetric_args, "ok"), plain_text).result.signature
...
@pytest.mark.parametrize(("session_asymmetric_key", "ok"), [
    ("expired", False),
    ("revoked", False),
    ("ok", True),
...
@pytest.mark.parametrize(("plain_text", "ok"), [
...
@pytest.mark.parametrize(("session_asymmetric_key"), [
    "expired",
    "revoked",
    "ok",
], indirect=True)
...
@pytest.mark.parametrize(("temp_asymmetric_key", "ok"), [
    ("ok", True),
    ("expired", False),
    ("revoked", False),
...
@pytest.mark.parametrize(("params", "ok"), [
    ({"public_key": "canonical_asymmetric_args['public_key']", "private_key": "canonical_asymmetric_args['private_key']"}, True),
