...
class TestAudit(unittest.TestCase):
    ...
    def test_search_custom_schema_order_by(self):
        limit = 2
        max_result = 3
        response_search = self.auditCustomSchema.search(
            query='message:""',
            order=SearchOrder.DESC,
            order_by="field_int",
            limit=limit,
    ...
    def test_export_download(self) -> None:
        export_res = self.audit_general.export(verbose=False)
        ...
        try:
            self.audit_general.poll_result(request_id=export_res.request_id)
        except pe.AcceptedRequestException as e:
            self.assertEqual(e.response.status, "Accepted")

        try:
            self.audit_general.download_results(request_id=export_res.request_id)
